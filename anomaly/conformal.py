...
from IntelliMaint.utils.Utils import validate_measure_str, validate_fit_input, validate_is_fitted, validate_get_input, validate_list_not_empty, validate_int_higher
from sklearn.neighbors import LocalOutlierFactor
import numpy as np


def get_strangeness(measure="median", k=10):
    validate_measure_str(measure)
    if measure == "median":
...
class Strangeness:
    ...
    def fit(self, X):
        validate_fit_input(X)
        self.X = X

    def predict(self, x):
        validate_is_fitted(self.is_fitted())
        validate_get_input(x)

    def pvalue(self, score):
        validate_is_fitted(self.is_fitted())
        validate_list_not_empty(self.scores)
